...
contract LiskGarden {
    ...
    function waterPlant(uint256 plantId) external {
        ...
        require(msg.sender == plant.owner, "Only the plant owner can water the plant");
        require(!plant.isDead, "Cannot water a dead plant");
    ...
    function harvestPlant(uint256 plantId) external {
        ...
        Plant storage plant = plants[plantId];
        require(plant.exists, "Plant does not exist");
        require(msg.sender == plant.owner, "Not the owner of the plant");
        require(!plant.isDead, "Plant is dead");
        updatePlantStage(plantId);
        require(plant.stage == GrowthStage.BLOOMING, "Plant is not in blooming stage");
        plant.exists = false;
